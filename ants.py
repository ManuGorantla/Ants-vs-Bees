...
class Ant(Insect):
    ...
    def __init__(self, health=1, duble=False):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while loc != None and loc.is_hive != True:
            if loc.bees and dist >= self.lower_bound and dist <= self.upper_bound:
                return random_bee(loc.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        beesLoc = (self.place.bees)[:]
        
        if (self.health <= amount):
            for x in beesLoc:
                x.reduce_health(self.damage + amount)
        else:
            for y in beesLoc:
                y.reduce_health(amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown != 0:
            ...
            return None
        if self.place.bees:
            closeBee = random_bee(self.place.bees)
            closeBee.reduce_health(closeBee.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for x in list(self.place.bees):
            x.reduce_health(self.damage)
# END Problem 9
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        exitPlace = self.place.exit

        while exitPlace:
            if exitPlace.ant:
                exitPlace.ant.double()

                if exitPlace.ant.ant_contained and exitPlace.ant.is_container:
                    exitPlace.ant.ant_contained.double()    
            
            exitPlace = exitPlace.exit
...
class GameState:
    ...
    def __init__(self, beehive, ant_types, create_places, dimensions, food=2):
        """Create an GameStafte for simulating a game.
